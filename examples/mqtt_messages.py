...
class Messages:
    ...
    def __init__(self, config, handlerclass):
        if config.mqttconfig["thisclient"]["version"] != self.__libversion:
            raise ValueError("The MQTT definition is not compatible with this library version.")

        #try:
            # This device
        self.__device = config.mqttconfig["thisclient"]["deviceid"]
        self.__version = config.mqttconfig["thisclient"]["version"]
        self.__user = config.mqttconfig["thisclient"]["username"]
        self.__password = config.mqttconfig["thisclient"]["password"]
        if "devicetypes" in config.mqttconfig["thisclient"]:
            self.__devicetypes = config.mqttconfig["thisclient"]["devicetypes"]
        else:
            self.__devicetypes = ""
        ...
        if self.__transport.lower() == "tcp":
            ...
            if "certfile" in config.mqttconfig["broker"]:
                if os.path.isfile(config.mqttconfig["broker"]["certfile"]):
        ...
        self.__publishqueues = config.mqttconfig["publishto"]

        # Queues to listen to
        self.__listenqueues = config.mqttconfig["subscribeto"]
    ...
    def __isrightdevicetype(self, message):
        ...
        """
        response = False
        if "devicetype" in message["mqttmessage"]:
            if message["mqttmessage"]["devicetypes"] == self.__devicetypes:
                response = True
            else:
                self.__log("Message not for this device Type.")
        return response
    ...
    def __makemessage(self, queuename, what, paramdict):
        ...
        print("__makemessage", messagejson)
        return messagejson
    ...
    def sendmessage(self, sendqueuename, what, paramdict):
        for queue in self.__publishqueues:
            if queue["name"] == sendqueuename:
                ...
                if len(message) > 0:
                    print(self.__publishqueues["name" == sendqueuename]["definition"]["topic"])
                    self.__client.publish(self.__publishqueues["name" == sendqueuename]["definition"]["topic"],
                                          message,
